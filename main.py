...
from base_train import ModelTrain


# Encode target labels using LabelEncoder
label_encoder = LabelEncoder()

df_train = pd.read_csv("./documents/suicidality_labeled_train.csv")
...
df_test = pd.read_csv("./documents/suicidality_labeled_test.csv")
...
model_roberta = ModelTrain('roberta-base', label_encoder, X_train, X_test, X_valid, y_train, y_test, y_valid, k_folds=5)
toc = time.perf_counter()
print(f"Trained Roberta model in {toc - tic:0.4f} seconds")

print("Distilbert:")
tic = time.perf_counter()
model_distilbert = ModelTrain('distilbert-base-uncased', label_encoder, X_train, X_test, X_valid, y_train, y_test, y_valid, k_folds=5)
toc = time.perf_counter()
print(f"Trained Distilbert model in {toc - tic:0.4f} seconds")

print("Albert:")
tic = time.perf_counter()
model_albert = ModelTrain('albert-base-v2', label_encoder, X_train, X_test, X_valid, y_train, y_test, y_valid, k_folds=5)
toc = time.perf_counter()
print(f"Trained Albert model in {toc - tic:0.4f} seconds")

print("Bert:")
tic = time.perf_counter()
model_bert = ModelTrain('bert-base-uncased', label_encoder, X_train, X_test, X_valid, y_train, y_test, y_valid, k_folds=5)
toc = time.perf_counter()
print(f"Trained Bert model in {toc - tic:0.4f} seconds")

print("Electra:")
tic = time.perf_counter()
model_electra = ModelTrain('google/electra-base-discriminator', label_encoder, X_train, X_test, X_valid, y_train, y_test, y_valid, k_folds=5)
toc = time.perf_counter()
